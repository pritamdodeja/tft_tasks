...
class TestTracePathInspectDecorator(unittest.TestCase):
    # {{{ TracePath Setup
    def setUp(self):
        self.EmptyTestTracePath = TracePath(name="EmptyTestTracePath", instrument=True)
...
def test_inspect_function_non_empty_execution(self):
    ...
    self.assertEqual(self.TestTracePath.function_mapping_list[0].caller, 'setUp')
    ...
    self.assertEqual(self.TestTracePath.function_mapping_list[1].caller, 'setUp')
    self.assertEqual(str(self.TestTracePath.function_mapping_list[0].args), '(2,)')
...
class TestTracePathGraphConstruction(unittest.TestCase):
    ...
    def test_construct_empty_graph(self):
        # Create the empty graph itself, construct_graph creates nodes and edges
        # We called no functions from this function, hence 0 nodes and 0 edges
        self.assertEqual(self.EmptyTestTracePath.graph.number_of_nodes(),0)
        self.assertEqual(self.EmptyTestTracePath.graph.number_of_edges(),0)
        self.assertRaises(KeyError, self.EmptyTestTracePath.graph.get_node, 'does_not_exist')
